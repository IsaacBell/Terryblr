module Terryblr::AdminHelper
  ...
  def add_inline_photo(url)
    ...
    link_to_function(t(:'.add_inline_photo'), "$('##{css_flash}').swfupload('selectFile')") +
  ...
  def sidebar_new_content_link
    ...
    link_to t("new_#{content_type}").capitalize, admin_new_content_path(content_type) if content_type
  end
