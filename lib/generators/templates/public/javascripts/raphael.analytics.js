...
function displayAnalyticsGraph() {
    $("#visitors_data, #tweets_data").each(function(i, el) {
      table = $(el)
      table.css({
          position: "absolute",
          left: "-9999em",
      ...
      var path = r.path().attr({stroke: color, "stroke-width": 4, "stroke-linejoin": "round"}),
          bgp = r.path().attr({stroke: "none", gradient: [90, color, color].join("-"), opacity: 0}).moveTo(leftgutter + X * .5, height - bottomgutter),
      ...
      label[1] = r.text(60, 40, date.toDateString()).attr(txt1).attr({fill: color}).hide();
...
function displayCountriesGraph() {
	// Raphael Charts
	pie_data = $("#countries_data")
  pie_holder = $("#countries_pie");
